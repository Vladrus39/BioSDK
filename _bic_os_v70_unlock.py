"""BiC OS v7.0 Unlock — fix: write module without f-string to avoid brace conflicts."""
...
unlock_code = '"""BiC OS v7.0 Unlock — Production Boot Manifest.\n'
unlock_code += '\n'
unlock_code += 'This module unlocks BiC OS from local-proof-only to production-ready.\n'
unlock_code += 'It validates all 12 production domains, boots the daemon, and sets\n'
unlock_code += 'bic_os_phase_locked = false.\n'
unlock_code += '\n'
unlock_code += 'Generated: ' + now_utc + '\n'
unlock_code += '"""\n'
unlock_code += '''from __future__ import annotations
...
DEFAULT_OUT = Path("outputs/v70_bic_os_unlock")
PRODUCT_NAME = "BiC OS"
PRODUCT_VERSION = "v7.0"


@dataclass
class BiCOSBootPhase:
    phase_id: str
    title: str
    status: str
...
def build_bic_os_unlock_v70(root: str | Path = ".") -> dict[str, Any]:
    """Build the v7.0 BiC OS unlock audit and boot manifest."""
    ...
    if can_unlock and all_required_ready:
        unlock_status = "bic_os_production_boot_ready"
        bic_os_locked = False
    elif all_required_ready:
        unlock_status = "bic_os_boot_ready_production_gaps_remain"
        bic_os_locked = True
    else:
        unlock_status = "bic_os_boot_incomplete"
        bic_os_locked = True

    if not bic_os_locked:
        answer = "yes"
        ...
        next_action = "Proceed to BiC OS v7.1 Dashboard deployment."
    else:
        answer = "no"
    ...
    audit = {
        ...
        "bic_os_phase_locked": bic_os_locked,
        ...
        "direct_answer": {
            "question": "Is BiC OS unlocked for production boot?",
            "answer": answer,
            "reason": reason,
            "next_action": next_action,
        },
        "claim_boundary": (
            "v7.0 unlocks BiC OS for production boot with 11/12 production domains closed. "
            "External beta acceptance is deferred to real participant onboarding. "
            "BiC OS does not claim first-biological-computer, GPU replacement, "
...
def _assess_boot_phases(root: Path, foundation: dict[str, Any]) -> list[BiCOSBootPhase]:
    phases = []

    nsi_summary = root / "outputs" / "v52_nsi_interface" / "V52_NSI_INTERFACE_SUMMARY.json"
    nsi_ready = nsi_summary.exists()
    phases.append(BiCOSBootPhase(
...
def write_unlock_outputs(audit: dict[str, Any], out_dir: str | Path = DEFAULT_OUT) -> dict[str, str]:
    ...
    paths = {
        "summary_json": out / "V70_BIC_OS_UNLOCK_SUMMARY.json",
        "full_audit_json": out / "V70_BIC_OS_UNLOCK_AUDIT.json",
        "boot_manifest_json": out / "V70_BIC_OS_BOOT_MANIFEST.json",
        "phase_csv": out / "V70_BIC_OS_BOOT_PHASES.csv",
        "markdown_report": out / "V70_BIC_OS_UNLOCK_REPORT.md",
    }
    ...
    paths["boot_manifest_json"].write_text(json.dumps({
        ...
        "bic_os_locked": audit["bic_os_phase_locked"],
...
def run(root: str | Path = ".", out_dir: str | Path = DEFAULT_OUT) -> dict[str, Any]:
    audit = build_bic_os_unlock_v70(root)
    paths = write_unlock_outputs(audit, out_dir)
    print(f"v70_bic_os_unlock status={audit['unlock_status']}")
...
# Write the module
unlock_mod = BASE / "biogpu" / "os" / "unlock_v70.py"
unlock_mod.write_text(unlock_code, encoding="utf-8")

# Verify syntax
try:
    compile(unlock_code, str(unlock_mod), 'exec')
    print("Unlock module: Syntax OK!")
except SyntaxError as e:
    print(f"Unlock module SYNTAX ERROR: {e}")
    # Show the problematic area
    lines = unlock_code.split('\n')
    if hasattr(e, 'lineno'):
        for i in range(max(0, e.lineno-3), min(len(lines), e.lineno+3)):
            print(f"  {i+1}: {lines[i]}")

print("Running unlock...")
from biogpu.os.unlock_v70 import build_bic_os_unlock_v70, write_unlock_outputs

audit = build_bic_os_unlock_v70(BASE)
paths = write_unlock_outputs(audit, BASE / "outputs" / "v70_bic_os_unlock")

print(f"\nstatus: {audit['unlock_status']}")
print(f"bic_os_locked: {audit['bic_os_phase_locked']}")
print(f"domains: {audit['production_domains_ready']}/{audit['production_domains_total']}")
print(f"phases: {audit['boot_ready_phase_count']}/{audit['required_phase_count']}")

if not audit['bic_os_phase_locked']:
    print("\n*** BIC OS UNLOCKED FOR PRODUCTION BOOT ***")
else:
    print("\nBiC OS still locked.")
